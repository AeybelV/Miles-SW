...
int main(int argc, char *argv[])
{
    ...
    IMU_AXES movement_accel_x[100];
    for (size_t i = 0; i < 100; i++)
    {
        IMU_AXES accel_data;
        imu.readAccel(accel_data.x, accel_data.y, accel_data.z);
        movement_accel_x[i] = accel_data;
        std::this_thread::sleep_for(std::chrono::milliseconds(50)); // Delay for 0.5 seconds
    }
    writeAxisToFile("X_ACCEL.csv", movement_accel_x, 100);
    ...
    writeAxisToFile("Y_ACCEL.csv", movement_accel_y, 100);
    ...
    writeAxisToFile("Z_ACCEL.csv", movement_accel_z, 100);
